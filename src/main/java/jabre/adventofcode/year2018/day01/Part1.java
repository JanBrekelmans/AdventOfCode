...
public class Part1 implements Solution {
    ...
    @Override
    public String solve() {
        Day1FileContents contents = getFileContents();
        return String.valueOf(contents.values.stream().mapToInt(i -> i).sum());
    }
    ...
    static Day1FileContents getFileContents() {
        ...
        try (BufferedReader reader = Files.newBufferedReader(new File(fileName.toURI()).toPath());
            ...
            return new Day1FileContents(lines.map(Integer::valueOf).toList());
        } catch (IOException | URISyntaxException e) {
            throw new RuntimeException(e);
        }
    }

    record Day1FileContents(List<Integer> values) {
    }
}
